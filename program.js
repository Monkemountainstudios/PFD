...
function createTrack(index) {
  ...
  const track = {
    ...
    loadAttempted: false,
    mode: 'variation',
    tune: 0, volume: 1, pan: 0, reverb: 0.08,
    gainNode: null, panNode: null, sendGain: null
  };
...
function synthFallback(trackIndex, time) {
  ...
  gain.connect(tracks[trackIndex].gainNode || audioCtx.destination);
...
function triggerTrack(track, time) {
  ...
  if (track.buffer) {
    ...
    source.connect(track.gainNode || audioCtx.destination);
    source.start(time);
  } else {
...
function scheduleStep(time) {
  ...
  if (level === 0) {
    tracks.forEach(track => { track.path = track.mode === 'static' ? [0,0,0,0] : makeRandomPath(); });
    flashHiddenTrackButtons(time);
  }
  ...
  tracks.forEach(track => {
    ...
    if (level < LEVELS - 1) {
      ...
      animateBranch(track, level + 1, childIndex, time, stepSeconds() * 0.96);
    }
  });
...
async function startSequencer() {
  ...
  stepIndex = 0;
  tracks.forEach(track => { track.path = null; });
...
channels.forEach((channel, i) => {
  const t = channel.querySelector('.tune'), v = channel.querySelector('.volume'), p = channel.querySelector('.pan'), r = channel.querySelector('.reverb');
  const apply = () => {
    const tr = tracks[i]; tr.tune=Number(t.value); tr.volume=Number(v.value)/100; tr.pan=Number(p.value)/100; tr.reverb=Number(r.value)/100;
  ...
  [t,v,p,r].forEach(el => el.addEventListener('input', apply));
  t.addEventListener('dblclick',()=>{t.value=0;apply();}); p.addEventListener('dblclick',()=>{p.value=0;apply();});
});
